@foreach ($modals as $proses)
    <div id="modalinfo{{ $proses->id }}"
        class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 hidden" style="overflow-y: auto;">
            ...
            <div class="card">
                            ...
                            <div class="divide-y divide-gray-200">
                                <div class="py-4 flex justify-between items-center">
                                    <div class="flex items-center space-x-4">
                                        <img src="{{ asset('file/produk/' . $modalcart->produk->foto[0]->foto) }}"
                                            alt="Produk" class="w-16 h-16 rounded-lg shadow">
                                        <div>
                ...
                <div class="mb-6">
                    ...
                    <div class="p-4 bg-gray-100 rounded-lg">
                        <p class="text-sm text-gray-600">{{$proses->catatan}}</p>
                    </div>
                </div>
                ...
                <div class="flex mt-3 gap-2">
                    @if ($proses->status == 'payment' || $proses->status == 'COD')
                        <form action="{{ route('proses', $proses->id) }}" method="POST">
                            @csrf
                            <button type="submit"
