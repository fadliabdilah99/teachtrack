...
@section('content')
    ...
    <div class="grid grid-cols-3 gap-4 mb-6">
        ...
        <button onclick="modalkelas()" class="bg-blue-400 rounded-lg p-4 flex items-center text-white">
            <i class="ti ti-layout-dashboard text-3xl mr-3"></i>
            <span class="font-semibold text-lg">kelas</span>
        </button>
        <button onclick="modalmapel()" class="bg-blue-500 rounded-lg p-4 flex items-center text-white">
        ...
        <button onclick="modallistkelas()" class="bg-blue-400 rounded-lg p-4 flex items-center text-white">
    ...
    <div id="editsiswa" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 hidden">
        <div class="bg-white rounded-lg shadow-lg w-11/12 md:w-1/2 lg:w-1/3 p-6">
            <!-- Header Modal -->
            <div class="flex justify-between items-center mb-4">
                <h2 class="text-xl font-bold">Edit Data</h2>
                <button onclick="modaleditclose()" class="text-gray-400 hover:text-gray-600">&times;</button>
            </div>
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('editsiswa') }}" method="POST">
                        @csrf
                        <input type="user_id" hidden name="user_id" id="edituser_id" hidden">
                        <div class="mb-6">
                            <label for="input-label-with-helper-text" class="block text-sm mb-2 text-gray-400">Nama</label>
                        ...
                        <div class="mb-6">
                            <label for="input-label-with-helper-text" class="block text-sm mb-2 text-gray-400">NIS</label>
                            <input type="number" name="NoUnik" id="editnik"
                                class="py-3 px-4 text-gray-500 block w-full border-gray-200 rounded-sm text-sm focus:border-blue-600 focus:ring-0 "
                                placeholder="" aria-describedby="hs-input-helper-text">
                        </div>
                        <!-- Footer Modal -->
                        <div class="flex justify-end">
                            <button type="button" onclick="modaleditclose()"
                                class="bg-gray-300 hover:bg-gray-400 text-gray-800 px-4 py-2 rounded mr-2">Cancel</button>
                            <button class="btn text-base py-2.5 text-white font-medium w-fit hover:bg-blue-700"
                                type="submit">Confirm</button>
                        </div>
...
@push('script')
    ...
    <script>
        ...
        // edit data siswa
        function modaledit(id, name, nik) {
            document.getElementById("editsiswa").classList.remove("hidden");
        ...
        function modaleditclose() {
            document.getElementById("editsiswa").classList.add("hidden");
        }
    </script>
@endpush
